...
contract functionsBasic {
    ...
    //constructor : constructor overloading is not supported. Only one construcctor is allowed.
   ...
   function getOwnerName_style3() returns(string name) {
    ...
    //function overloadding - funcction with same name but different input parameters
    
    function getOwnerInfo() returns (string name, uint age) {
        name = ownerName;
        age = ownerAge;
    }    
    
    function getOwnerInfo(uint limit) returns (string name, uint age) {
        if(limit > ownerAge)
            age = 12;
         else
            age = ownerAge;
