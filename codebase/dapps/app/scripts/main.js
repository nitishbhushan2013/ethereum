...
function doGetNodeSyncStatus() {
//Async function 

web3.net.isSyncing(function(error,result){
    alert(111);
    if(error) setData('node_syncing_status',error,true);
    else {
        setData('Node is syncing',result,false);
    }
});

}

function doGetNodeMiningStatus() {
 web3.eth.getPeerCount(function(error,result){
    if(error) setData('node_mining_status',error,true);
    else 
        setData('Node is mining',result,false);
 }); 
